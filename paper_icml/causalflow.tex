...
\begin{table}[t]
\caption{Repair performance across benchmarks. Repair Rate is computed over failed traces only.}
...
Causal clustering reveals domain-specific skill categories.
...
These clusters demonstrate that causal attribution exposes interpretable and structured behavioral weaknesses rather than arbitrary error patterns.
...
\midrule
GSM8K & \textit{[fill]} & \textit{[fill]} \\
MBPP & \textit{[fill]} & \textit{[fill]} \\
SealQA Hard & \textit{[fill]} & \textit{[fill]} \\
MedBrowseComp & \textit{[fill]} & \textit{[fill]} \\
\bottomrule
\end{tabular}
\end{small}
\end{table}
